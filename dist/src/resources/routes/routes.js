...
function getAll(resource) {
    return async (request, reply) => {
        try {
            ...
            (0, events_1.emitAction)(resource.model, HandlerType.GET_ALL, null, data.rows);
        }
        catch (err) {
            reply.log.error(err);
            reply.status(500).send({ error: `Failed to fetch ${resource.name}.` });
            (0, events_1.emitAction)(resource.model, HandlerType.GET_ALL, err);
        }
    };
}
...
function getOne(resource) {
    return async (request, reply) => {
        try {
            ...
            reply.send(data);
            (0, events_1.emitAction)(resource.model, HandlerType.GET_ONE, null, values.rows);
        }
        catch (err) {
            reply.log.error(err);
            reply.status(500).send({ error: `Failed to fetch ${resource.name}.` });
            (0, events_1.emitAction)(resource.model, HandlerType.GET_ONE, err);
        }
    };
}
...
function create(resource) {
    return async (request, reply) => {
        try {
            ...
            (0, events_1.emitAction)(resource.model, HandlerType.CREATE, null, data);
        }
        catch (err) {
            reply.log.error(err);
            reply.status(500).send({ error: `Failed to create ${resource.name}.` });
            (0, events_1.emitAction)(resource.model, HandlerType.CREATE, err);
        }
    };
}
...
function update(resource) {
    return async (request, reply) => {
        try {
            ...
            reply.send(data);
            (0, events_1.emitAction)(resource.model, HandlerType.UPDATE, null, value.rows);
        }
        catch (err) {
            reply.log.error(err);
            reply.status(500).send({ error: `Failed to update ${resource.name}.` });
            (0, events_1.emitAction)(resource.model, HandlerType.UPDATE, err);
        }
    };
}
...
function remove(resource) {
    return async (request, reply) => {
        try {
            ...
            (0, events_1.emitAction)(resource.model, HandlerType.REMOVE, null, value.rows);
        }
        catch (err) {
            reply.log.error(err);
            reply.status(500).send({ error: `Failed to delete ${resource.name}.` });
            (0, events_1.emitAction)(resource.model, HandlerType.REMOVE, err);
        }
    };
}
