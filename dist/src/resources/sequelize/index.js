    ...
    for (const sequelizeResource of sequelizeResources) {
        ...
        for (const columnName of Object.keys(attributes)) {
            ...
            if (ResourceValues !== undefined) {
                if (attrType instanceof sequelize_1.DataTypes.ENUM) {
                    resource.columns[columnName].values = ResourceValues;
                }
                else if (attrType instanceof sequelize_1.DataTypes.STRING) {
                    resource.columns[columnName].maxLength = ResourceValues;
                }
            }
            if (search === true) {
            ...
            if (resource.columns[columnName].include !== undefined) {
                resource.include.push({
                    model: resource.columns[columnName].include,
                    as: singularize((resource.columns[columnName].include?.getTableName()).toLowerCase())
                });
            }
        }
...
function dataTypesResultToResourceType(data) {
    ...
    else if (data instanceof sequelize_1.DataTypes.ARRAY) {
        return ResourceType.ARRAY;
    }
    throw new Error(`Unknown column type: ${data}`);
}
function dataTypesResultToResourceValues(data) {
    if (data instanceof sequelize_1.DataTypes.ENUM) {
        const dataEnum = data;
        return dataEnum.values;
    }
    else if (data instanceof sequelize_1.DataTypes.STRING) {
